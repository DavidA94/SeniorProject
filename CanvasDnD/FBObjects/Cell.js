...
class Cell extends TextBlock {
    constructor(text){
        ...
        this.layout.padding.top = this.layout.padding.right =
            this.layout.padding.bottom = this.layout.padding.left = TABLE_BORDER_SIZE;
    ...
    draw(context) {
        ...
        const shiftAmt = TABLE_BORDER_SIZE / 2;
        context.lineWidth = TABLE_BORDER_SIZE;
        context.strokeStyle = this.borderColor;
        context.strokeRect(this.layout.x + shiftAmt, this.layout.y + shiftAmt,
            this.layout.width - TABLE_BORDER_SIZE, this.layout.height - TABLE_BORDER_SIZE);

        if(this.isFocused){
            // Draw another border if we're focused
            const shiftAmt = (TABLE_BORDER_SIZE / 2) + TABLE_BORDER_SIZE;
            context.lineWidth = 2 * TABLE_BORDER_SIZE;
