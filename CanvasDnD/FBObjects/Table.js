...
class Table extends FBObject {
    ...
    constructor(x, y, width, height){
        ...
        const horizontalSep = new Box(0, 0, width, 2 * TABLE_BORDER_SIZE);
        ...
        this.border.top = TABLE_BORDER_SIZE;
        this.border.right = TABLE_BORDER_SIZE;
        this.border.bottom = TABLE_BORDER_SIZE;
        this.border.left = TABLE_BORDER_SIZE;
        ...
        this.layout.subscribe(EVENT_PROPERTY_CHANGE, (e) => {
            if(e.propertyName == "width"){
                for(let separator of this._separators){
                    // horizontal bar
                    if(separator.layout.height == 2 * TABLE_BORDER_SIZE){
                ...
                for(let separator of this._separators){
                    // vertical bar
                    if(separator.layout.width == 2 * TABLE_BORDER_SIZE){
    ...
    _addSeparator(column){
        const newSep = new Box(0, 0, TABLE_BORDER_SIZE * 2, this.height);
    ...
    _doDraw(context){
        ...
        if(this._separators.length > 0) {
            this._separators[0].layout.y = this.y + this.headerHeight - TABLE_BORDER_SIZE;
            ...
            for (let i = 1; i < this._separators.length; ++i) {
                ...
                this._separators[i].layout.x = this.x + currentXPos - TABLE_BORDER_SIZE;
    ...
    _separator_MouseDown(e){
        ...
        // If it's vertical
        if(e.sender.layout.width === 2 * TABLE_BORDER_SIZE){
